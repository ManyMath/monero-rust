...
use monero_serai::{
    ...
    wallet::{
        seed::Seed,
        address::{Network, AddressSpec, SubaddressIndex},
        ViewPair,
    },
};
...
fn test_account_0_subaddress_0_is_standard_address() {
    ...
    assert_eq!(standard_address.to_string(), EXPECTED_ACCOUNT_0_SUBADDRESS_0);
...
fn test_account_1_subaddress_0_derivation() {
    ...
    let subaddress_index = SubaddressIndex::new(1, 0)
        .expect("SubaddressIndex::new(1, 0) should return Some");

    // Derive the subaddress
    let subaddress = pair.address(
        Network::Stagenet,
        AddressSpec::Subaddress(subaddress_index)
    );
...
fn test_account_0_subaddress_1_is_different() {
    ...
    let subaddress_index = SubaddressIndex::new(0, 1)
        .expect("SubaddressIndex::new(0, 1) should return Some");
...
fn test_account_1_subaddress_1_derivation() {
    ...
    let subaddress_index = SubaddressIndex::new(1, 1)
        .expect("SubaddressIndex::new(1, 1) should return Some");
