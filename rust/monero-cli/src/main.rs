...
async fn cmd_sync(daemon: &str, wallet_path: Option<&str>) -> Result<(), String> {
    ...
    // Get daemon height to set up progress bar
    ...
    loop {
        ...
        // Fetch a batch of blocks (~1000 at a time)
        let fetched =
        ...
        // Process the fetched batch, reusing the cached scanner
        ...
        // Process batch results: collect outputs and track spent key images
        ...
        // Mark spent outputs based on key images seen on-chain
...
async fn cmd_transfer(
    ...
    // Get spendable outputs (unspent and unlocked)
    let daemon_height = monero_rust::get_daemon_height(daemon).await?;
    ...
    // Select inputs via coin selection
    ...
    // Prepare the transaction to get the fee estimate
    ...
    // Display fee and ask for confirmation
    ...
    // Flush stdout so the prompt is visible before reading
    use std::io::Write;
    std::io::stdout()
        .flush()
    ...
    // Add change outputs back to wallet
...
async fn cmd_sweep_all(
    ...
    // Estimate fee to display to user
    ...
    // Mark all swept outputs as spent
