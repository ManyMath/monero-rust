//! Spent output detection using key image checking
...
/// Check if key images are spent by querying the daemon
/// Returns a vector of spent statuses matching the input order
pub async fn check_key_images_spent<R: RpcConnection>(
    ...
    // Call the is_key_image_spent RPC endpoint
    // The Monero daemon's is_key_image_spent method takes an array of key images
    ...
    struct IsKeyImageSpentResult {
        spent_status: Vec<u32>, // 0 = unspent, 1 = spent in pool, 2 = spent in blockchain
    }
    ...
    let results = key_images_hex
        ...
        .map(|(key_image, &status)| KeyImageSpentStatus {
            ...
            spent: status > 0, // Consider both pool and blockchain as spent
        })
        .collect();
...
/// Batch key images into smaller chunks for efficient checking
/// Recommended batch size is 100-1000 depending on network conditions
...
/// Helper function for native (non-WASM) environments to check spent status
#[cfg(not(target_arch = "wasm32"))]
pub async fn check_key_images_spent_native(
    ...
    // Call the is_key_image_spent JSON-RPC endpoint
    let params = json!({
